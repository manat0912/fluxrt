...
def _flux2_kv_causal_attention(
    ...
    if kv_cache is not None:
        ...
        k_all = torch.cat(
            [key[:, :num_txt_tokens], k_ref, key[:, num_txt_tokens:]], dim=1
        )
        v_all = torch.cat(
            [value[:, :num_txt_tokens], v_ref, value[:, num_txt_tokens:]], dim=1
        )
...
class Flux2KVAttnProcessor:
    ...
    def __call__(
        ...
        if kv_cache_mode == "extract" and kv_cache is not None and num_ref_tokens > 0:
            ...
            ref_end = num_txt_tokens + num_ref_tokens
            kv_cache.store(
                key[:, ref_start:ref_end].clone(), value[:, ref_start:ref_end].clone()
            )
